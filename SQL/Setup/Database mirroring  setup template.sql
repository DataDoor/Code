...
use template values replace require values

*/

/*Check the principal database is currently in FULL recovery mode*/

SELECT Name, recovery_model_desc FROM sys.databases WHERE name = N'<Database_Name, sysname, Database_Name>'


/*If not set the database to full recovery*/

:connect <Principal_DB_Instance, sysname, Principal_DB_Instance>
ALTER DATABASE <Database_Name, sysname, Database_Name>  SET RECOVERY FULL WITH NO_WAIT;
GO
...
TO DISK = N'<Backup_Path, sysname, \\EM-APPL018\ITSQL\>'
GO
...
BACKUP LOG <Database_Name, sysname, Database_Name> 
TO DISK = N'<Backup_Path, sysname, \\EM-APPL018\ITSQL\>'
...
FROM DISK = N'<Backup_Path, sysname, \\EM-APPL018\ITSQL\>'

/*Restore the database but ensure that its left in a recovering state for mirroring*/

:connect <Mirror_DB_Instance, sysname, Mirror_DB_Instance>

/*Thisis onlect required if your restoring an exisiting database*/
---ALTER DATABASE <Database_Name, sysname, Database_Name> SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
...
:connect <Principal_DB_Instance, sysname, Principal_DB_Instance>
GRANT CONNECT ON ENDPOINT ::<Principal_DB_Instance, sysname, Principal_DB_Instance>_Mirroring_Endpoint --I.e [DBL01_Mirroring_Endpoint ]
...
:connect <Mirror_DB_Instance, sysname, Mirror_DB_Instance>
GRANT CONNECT ON ENDPOINT ::<Mirror_DB_Instance, sysname, Mirror_DB_Instance>_Mirroring_Endpoint  --I.e [DBT01_Mirroring_Endpoint ]
...
ALTER DATABASE <Database_Name, sysname, Database_Name>  SET PARTNER = N'TCP://<Principal_DB_Instance, sysname, Principal_DB_Instance>.Emerald.Net:5022' --TCP for Principal i.e. TCP://DBL01.Emerald.Net:5022
...
ALTER DATABASE <Database_Name, sysname, Database_Name>  SET PARTNER = N'TCP://<Mirror_DB_Instance, sysname, Mirror_DB_Instance>.Emerald.Net:5022' ---TCP for Mirror Instance i.e. N'TCP://DBT01_Instance.Emerald.Net:5022' 
...
ALTER DATABASE <Database_Name, sysname, Database_Name>  SET PARTNER FAILOVER;



/*To stop the mirroring all together either do this through the Mirroring configuration in SSMS or T-SQL*/ 
 
ALTER DATABASE <Database_Name, sysname, Database_Name>  SET PARTNER OFF 
